  ...
  try {
            ...
            if (!sessionId && !sessionCreatedSent) {
              ...
              try {
                ...
                if (project) {
                  ...
                  // CodeBuddy sessions are stored in project-specific directories
                  const codebuddyProjectName = projectPath.replace(/^\//, '').replace(/\//g, '-');
                  const codebuddyProjectDir = path.join(os.homedir(), '.codebuddy', 'projects', codebuddyProjectName);
                  const sourceFile = path.join(codebuddyProjectDir, `${capturedSessionId}.jsonl`);
                  
                  createSession(project.id, capturedSessionId, 'codebuddy', null, sourceFile);
    ...
    if (capturedSessionId) {
      ...
      try {
        ...
        console.error('❌ [CodeBuddy SDK] Failed to update session on completion:', dbError);
      }
    }
